...
class Crunchyroll : Platform {
    ...
    override fun getLastNews(): MutableList<News> {
        ...
        val doc = db.parse(URL("https://www.crunchyroll.com/newsrss?lang=frFR").openConnection().getInputStream())
        ...
        for (i in 0 until list.length) {
            ...
            if (node.nodeType == Node.ELEMENT_NODE) {
                ...
                if (this.isSameDay(calendar, releaseDate)) l.add(
                    News(
                        this,
                        releaseDate,
                        title,
    ...
    override fun getLastEpisodes(): MutableList<Episode> {
        ...
        val db = dbf.newDocumentBuilder()
        val doc = db.parse(
            URL("https://feeds.feedburner.com/crunchyroll/rss/anime?lang=frFR").openConnection().getInputStream()
        )
        doc.documentElement.normalize()
        val list = doc.getElementsByTagName("item")
        ...
        for (i in 0 until list.length) {
            ...
            if (node.nodeType == Node.ELEMENT_NODE) {
                ...
                if (spay.split(" ").contains("fr") && this.isSameDay(calendar, releaseDate)) {
