...
class AnimeDigitalNetworkPlatform : IPlatform("Anime Digital Network") {
    ...
    private fun toEpisode(json: JsonObject): Episode {
    ...
    override fun getResponse(calendar: Calendar): Network.NetworkResponse = Network.connect("https://gw.api.animedigitalnetwork.fr/video/calendar?date=${calendar.toISODate()}")

    override fun toEpisode(episode: String): Episode {
        val json = try { gson.fromJson(episode, JsonObject::class.java) } catch (e: Exception) { null } ?: throw MalformedEpisodeException("Could not parse JSON")
        return toEpisode(json)
    }

    override fun getAllEpisodes(calendar: Calendar): Collection<Episode> {
        val networkResponse = getResponse(calendar)
